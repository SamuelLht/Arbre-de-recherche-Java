...
public class arbre {
	...
	public noeud recherche(noeud racine, int c) {
		...
		else if (racine.cle[0] > c) { // si c est inf�rieur a la premiere valeur
			if (racine.enfant[0] == null || racine.enfant[0].cle[0] == -1) { // on tcheck si lenfant est null
				return racine;
			} else {
				return recherche(racine.enfant[0], c);
			}
		} else if (racine.cle[1] < c) { // si c est sup�rieur � la derniere valeur
	...
	public void insertion(noeud racine, int c){
		...
		if (racine.cle[1] == -1 && c > racine.cle[0]) { // si le noeud contient qu'une valeur et que c est sup�rieur
			racine.cle[1] = c;
